...
#include "vertexshaderblock.h"
#include "data/blocks/portlist.h"
#include "data/blocks/port.h"

namespace ysm
{
	VertexShaderBlock::VertexShaderBlock(Pipeline* parent) : ShaderBlock(parent, block_type, "Vertex Shader")
	{

	}

	Port* VertexShaderBlock::getTransformFeedbackOutPort()
	{
		return _transformFeedbackOutPort;
	}

	void VertexShaderBlock::createPorts()
	{
    ...
    bool VertexShaderBlock::canAcceptConnection(IPort* src, IPort* dest, QString& denialReason)
    {
        ...
        if (src == _outPort)
        {
            ...
            conPoints << qMakePair(BlockType::Shader_TessellationControl, PortType::GenericIn);
			...
			{
                denialReason = "Vertex Shader output must be connected to a Geometry Shader, Tessellation Control Shader, Tessellation Primitve Generator or Rasterization block";
				return false;
			}
        ...
        if (src == _transformFeedbackOutPort)
        {
            ...
            if (!checkConnectionPoints(dest, conPoints))
            {
                denialReason = "Vertex Shader Transform Feedback output must be connected to a Transform Feedback block";
                return false;
            }
