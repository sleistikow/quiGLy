...
QList<QString> GLSLUBODeclarationCheck::PACKAGE_QUALIFIERS;

GLSLUBODeclarationCheck::GLSLUBODeclarationCheck()
{
	...
	registerIntegrityCheck(&GLSLUBODeclarationCheck::ensureHasBody, true);
	registerIntegrityCheck(&GLSLUBODeclarationCheck::checkLocationIntegrity, false);
	registerIntegrityCheck(&GLSLUBODeclarationCheck::checkVersionIntegrity, false);
}

bool GLSLUBODeclarationCheck::isHandled(GLSLDeclaration* declaration) const
{
	//Must be uniform without type.
	return declaration->hasQualifier("uniform") && declaration->getDataType().isEmpty();
}

QList<IConnection*> GLSLUBODeclarationCheck::beginPipelineParse(IBlock* pipelineBlock, GLSLCodeBlock* codeBlock)
{
	Q_UNUSED(codeBlock)
	...
	//Use UBO ins.
	foreach(IConnection* connection, pipelineBlock->getPort(PortType::Shader_UBO)->getInConnections())
...
void GLSLUBODeclarationCheck::extractElementData(IConnection* pipelineElement, GLSLCodeBlock* codeBlock)
{
...
void GLSLUBODeclarationCheck::storeDocumentData(IConnection* pipelineElement, GLSLCodeBlock* codeBlock,
...
QString GLSLUBODeclarationCheck::getElementName(IConnection* pipelineElement)
{
	//Get the connection's name property.
	return pipelineElement->getProperty<StringProperty>(PropertyID::Buffer_Name)->getValue();
}

IGLSLPipelineCheck::FixedStatement GLSLUBODeclarationCheck::getElementStatement(IConnection* pipelineElement) const
{
	Q_UNUSED(pipelineElement);

	//Create the content string and layout specifier.
	QString instanceName;
	QString packageQualifier = "std140";
	...
	QStringList elementCode = QStringList()
		<< QString("layout(%1%5%2%3) uniform %4")
...
bool GLSLUBODeclarationCheck::checkLocationIntegrity(IConnection* connection, GLSLDeclaration* declaration)
{
...
bool GLSLUBODeclarationCheck::checkVersionIntegrity(IConnection* connection, GLSLDeclaration* declaration)
{
	Q_UNUSED(connection)

	//Get the code location and ensure it's correct.
	//TODO: Minimum GLSL version, not OpenGL version.
	int minimumVersion = 140; //connection->getDestPort()->getMinimumVersion();
	if(_pipelineVersion < minimumVersion)
	{
		declaration->addPipelineError(
		{
			QString("Uniform buffer objects not supported by GLSL version %1 (minimum required version: %2).")
