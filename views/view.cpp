...
{
	//Inherit the active item and document.
	if(getParentView())
	{
		//Do not use the setters, because they would call subclass methods from the constructor.
		_activeDocument = getParentView()->getActiveDocument();
		_activeItem = getParentView()->getActiveItem();
	}
}

bool View::inheritsActiveDocument() const { return true; }
bool View::inheritsActiveItem() const { return true; }
